...
export const WorkerDashboard = ({ onBulkUpload, onNavigateTab }) => {
  ...
  return (
    <div className="space-y-3 sm:space-y-3.5 animate-fadeIn pb-20 max-w-[1600px] mx-auto px-1.5 sm:px-0">
      {/* ========================================================================= */}
      {/* ⭐ [사용자 요청] 조인주 선임 접속 시: 1. 업무일지 작성 칸 & 2. 매입매출 엑셀 드래그 업로드 창 */}
      {/* ========================================================================= */}
      {isInjoo && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-1">
          {/* PANE 1: 오늘의 업무일지 작성 칸 */}
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 border-2 border-blue-500/30 dark:border-blue-500/20 shadow-sm flex flex-col justify-between space-y-3">
            ...
            <form onSubmit={handleSaveLog} className="space-y-2.5 text-xs">
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-2.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-[11px] font-bold text-slate-800 dark:text-slate-200"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500"
                ></textarea>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 sm:p-5 border-2 border-emerald-500/30 dark:border-emerald-500/20 shadow-sm flex flex-col justify-between space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold">
                    손익 매입-매출 정리본 및 원가 엑셀 파일 (.xlsx, .xls)
                  </span>
                </div>
              </div>
              {parsedResult && (
            ...
            {!parsedResult ? (
              <div
                ...
                className={`border-2 border-dashed rounded-2xl p-5 text-center cursor-pointer transition-all flex flex-col items-center justify-center gap-2 min-h-[160px] ${
                  dragActive
                    ? "border-emerald-500 bg-emerald-50/60 dark:bg-emerald-950/40 scale-[1.01]"
                    : "border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 hover:border-emerald-400 hover:bg-emerald-50/20"
                }`}
              >
                <div className="w-10 h-10 rounded-2xl bg-emerald-100 dark:bg-emerald-900/60 text-emerald-600 dark:text-emerald-300 flex items-center justify-center shadow-sm">
                  {parsing ? (
              ...
              <div className="space-y-3">
                <div className="p-3 rounded-xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200/80 dark:border-emerald-800 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <FileCheck className="w-5 h-5 text-emerald-600 flex-shrink-0" />
                    <div className="text-left min-w-0">
                      <span className="font-black text-xs text-slate-900 dark:text-white block truncate max-w-[180px]">
      ...
      {/* Write Work Log Modal (For other workers) */}
      {isModalOpen && (
