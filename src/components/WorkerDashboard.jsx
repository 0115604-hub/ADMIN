...
export const WorkerDashboard = ({ onBulkUpload, onNavigateTab }) => {
  ...
  return (
    <div className="space-y-3 sm:space-y-3.5 animate-fadeIn pb-20 max-w-[1600px] mx-auto px-1.5 sm:px-0">
      {/* ========================================================================= */}
      {/* TOP COMPACT HEADER (No top '작성' button per user request) */}
      {/* ========================================================================= */}
      <div className="bg-white dark:bg-slate-900 px-3.5 py-2.5 sm:px-4 sm:py-3 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-1.5 min-w-0">
            ...
            <span className="text-[11px] text-slate-400 font-semibold hidden md:inline">
              • 기준: <strong>{monthTitle}</strong> (일일생산정보현황 요약)
            </span>
          </div>
        </div>

        {/* Uploader Toggle if Injoo */}
        {isInjoo && (
          <button
            onClick={() => setActiveWorkerTab(activeWorkerTab === "uploader" ? "summary_log" : "uploader")}
            className="flex items-center gap-1 px-2.5 py-1 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-black shadow-sm shrink-0"
          >
            <UploadCloud className="w-3.5 h-3.5" />
            <span>{activeWorkerTab === "uploader" ? "종합 현황" : "엑셀 업로드"}</span>
          </button>
        )}
      </div>
