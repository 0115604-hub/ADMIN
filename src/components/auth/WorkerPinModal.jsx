    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* 🌟 1. Header: 작업자 이름과 직책 */}
          <div className="p-3.5 sm:p-4 md:px-6 md:py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 shrink-0 bg-slate-50/80 dark:bg-slate-900/80">
            <div className="flex items-center gap-3 min-w-0">
              <div className={`w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-2xl text-white flex items-center justify-center font-black text-lg md:text-xl shadow-md shrink-0 ${
                isAdmin
          ...
          {/* Admin Switcher Mobile (If Admin Role) */}
          {isAdmin && (
            <div className="sm:hidden px-3.5 pt-2 shrink-0">
          ...
          {/* 🌟 2. Body: PC에서는 좌우 2단 분할(Side-by-Side), 모바일에서는 1단으로 꽉 차게 노출 */}
          <div
            ref={bodyRef}
          ...
          >
            ...
            {leaveStatus && (
              <div className="p-2.5 sm:p-3 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800/80 flex items-center justify-between gap-2 text-xs shadow-2xs animate-pulse">
                ...
                <span className="px-2 py-0.5 rounded-lg bg-rose-600 text-white font-black text-[10px] sm:text-xs shrink-0">
                  {leaveStatus.displayBadge || "근태"}
                </span>
              </div>
            )}

            {/* PC 2-Column Grid / Mobile 1-Column */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 items-stretch">
              {/* [좌측 패널] 🚨 이명재 이사 중대재해 및 안전 공유 사진 */}
              <div className="p-3.5 sm:p-4 md:p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 via-rose-500/5 to-amber-500/10 dark:from-amber-950/30 dark:via-rose-950/20 dark:to-amber-950/30 border-2 border-amber-300/90 dark:border-amber-700/80 flex flex-col justify-between space-y-3 shadow-xs min-h-[260px] md:min-h-[320px]">
                <div className="space-y-3">
                  ...
                  {disasterPhotos.length > 0 ? (
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-2.5">
                        {disasterPhotos.slice(0, 6).map((photo) => (
                          <div
                            key={photo.id}
                      ...
                      {disasterPhotos.length > 6 && (
                        <div className="text-right pt-1">
                          <span className="text-xs font-bold text-amber-800 dark:text-amber-300">
                            외 {disasterPhotos.length - 6}장의 안전 사진이 등록되어 있습니다. (클릭 시 확대)
                          </span>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div className="py-12 text-center text-xs sm:text-sm text-amber-800/80 dark:text-amber-300/80 font-bold bg-white/60 dark:bg-slate-900/60 rounded-2xl border border-dashed border-amber-300 dark:border-amber-700/60 flex flex-col items-center justify-center">
                      <span>공유된 중대재해·안전 사진이 없습니다.</span>
                    </div>
                  )}
                </div>

                <div className="pt-2 border-t border-amber-300/40 dark:border-amber-700/40 flex items-center justify-between text-[11px] font-bold text-amber-800/90 dark:text-amber-300/90">
                  <span>게시자: 이명재 이사</span>
                  <span>사진 탭 시 고화질 원본 확대</span>
                </div>
              </div>

              {/* [우측 패널] 📢 전사 공지 및 실시간 공유 내용 */}
              <div className="p-3.5 sm:p-4 md:p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border-2 border-slate-200 dark:border-slate-700 flex flex-col justify-between space-y-3 shadow-xs min-h-[260px] md:min-h-[320px]">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                  ...
                  {displayNotices.length > 0 ? (
                      ...
                      {displayNotices.map((item) => {
                        ...
                        return (
                          <div
                            ...
                            className="p-2.5 sm:p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs space-y-1 hover:border-blue-400 dark:hover:border-blue-600 transition-colors"
                          >
                            ...
                            {item.title && item.content && (
                              <p className="text-[11.5px] sm:text-xs text-slate-600 dark:text-slate-300 line-clamp-2 font-medium leading-relaxed">
                                {item.content}
                              </p>
                            )}
                          </div>
                    ...
                    <div className="py-12 text-center text-xs sm:text-sm text-slate-400 font-bold bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center">
                      <span>등록된 사내 공유사항이 없습니다.</span>
                    </div>
                  )}
                </div>
                ...
                <div className="pt-2 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between text-[11px] font-bold text-slate-400">
                  <span>전사 실시간 공지 및 안건</span>
                  <span>상세 대장 조회 지원</span>
                </div>
              </div>
            </div>
          </div>
