      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden p-6 sm:p-8 my-8 relative">
        ...
        {!selectedUser ? (
          <div className="space-y-6">
            ...
            {/* 3. BOTTOM: ADMIN & Mobile QR Code */}
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-center gap-2">
              <button
                type="button"
                onClick={() => setShowQr(true)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50/70 dark:bg-blue-950/40 hover:bg-blue-100 text-blue-700 dark:text-blue-300 text-xs font-black transition-all shadow-sm group active:scale-95"
              >
                <QrCode className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
                <span>스마트폰 접속 QR</span>
              </button>
          ...
          <div className="space-y-4 text-center animate-fadeIn">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                카메라를 켜고 QR코드를 비추면 즉시 접속됩니다.
              </p>
            </div>
            <div className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-[11px] font-mono text-slate-500 dark:text-slate-400 break-all select-all">
              https://profit-and-loss-7d09b.web.app
            </div>
            <button
              type="button"
              onClick={() => setShowQr(false)}
              className="w-full py-2.5 rounded-xl bg-slate-900 dark:bg-slate-700 hover:bg-slate-800 text-white font-bold text-xs transition-colors"
            >
              사용자 선택으로 돌아가기
            </button>
          </div>
