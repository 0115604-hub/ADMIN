    ...
    <div className="fixed inset-0 z-50 overflow-y-auto overflow-x-hidden bg-slate-950/85 backdrop-blur-xl animate-fadeIn p-2 sm:p-4 py-2 sm:py-8 flex justify-center items-start min-h-screen max-w-full">
      ...
      <div className="bg-white/95 dark:bg-slate-900/90 w-full max-w-2xl sm:max-w-3xl md:max-w-4xl lg:max-w-5xl rounded-2xl sm:rounded-3xl shadow-[0_25px_70px_-15px_rgba(0,0,0,0.5)] border border-slate-200/80 dark:border-slate-800 backdrop-blur-2xl overflow-hidden my-auto relative animate-scaleUp min-w-0">
        ...
        <div className="p-3.5 sm:p-6 sm:px-7">
            ...
            {isIssueExpanded && displayedActiveIssues.length > 0 && (
              <div className="p-2 sm:p-3 space-y-2">
                {displayedActiveIssues.map((item) => {
                  ...
                  // 🌟 [오픈이슈] 회의일정 밑에 표시되는 한줄짜리 패널 (진행상황 그래프 + 맨 오른쪽 의견횟수 배지)
                  if (isOpenIssue) {
                    return (
                      <div
                        key={item.id}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 min-w-0 flex-1">
                          <span className="px-2 py-0.5 rounded-md text-[10.5px] font-black bg-gradient-to-r from-blue-600 to-indigo-600 text-white shrink-0 shadow-2xs flex items-center gap-1">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-[10px] font-black shrink-0 ${
                            ...
                            {item.plant?.replace("공장", "") || item.plant}
                          </span>
                          {item.expireDate && (
                            <span className="px-1.5 py-0.5 rounded text-[10px] font-mono font-bold shrink-0 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hidden sm:inline-block">
          ...
          {/* ⚡ ⭐ 실시간 변동사항이 있는 관리자 상태 (점멸 안내 패널) */}
          {managerLeaves.length > 0 && (
            <div className="mb-3.5 sm:mb-4.5 p-2.5 sm:p-3.5 rounded-2xl bg-gradient-to-r from-amber-500/15 via-rose-500/15 to-amber-500/15 dark:from-amber-950/50 dark:via-rose-950/40 dark:to-amber-950/50 border-2 border-amber-400 dark:border-amber-600 shadow-md animate-pulse space-y-2">
          ...
          {!selectedUser ? (
            ...
            <form onSubmit={handlePinSubmit} className="space-y-4 animate-fadeIn">
              ...
              {/* Hero Spotlight Profile Card (이름/프로필 탭 시 핀 입력창으로 즉시 포커스) */}
              <div
                onClick={() => {
                  const el = document.getElementById("worker-pin-input");
                  if (el) el.focus();
                }}
                className="p-4 rounded-2xl bg-gradient-to-r from-slate-50 to-blue-50/30 dark:from-slate-800/80 dark:to-blue-950/30 border-2 border-blue-200 dark:border-blue-900/60 hover:border-blue-400 dark:hover:border-blue-500 flex items-center justify-between cursor-pointer transition-all shadow-xs group"
                title="이름을 탭하여 PIN 번호를 입력하세요"
              >
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-2xl text-white flex items-center justify-center font-black text-lg shadow-md group-hover:scale-105 transition-transform ${
                    selectedUser.role === "ADMIN"
                  ...
                  <div>
                    <h4 className="font-black text-lg text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white font-bold px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xs hover:bg-slate-100 dark:hover:bg-slate-700 transition-all cursor-pointer active:scale-95"
                >
