...
export const HanulExpenseSummaryModal = ({
  ...
  const items = STANDARD_EXPENSE_CATEGORIES.map((cat, idx) => {
  ...
  const totalExpense = settlementData?.totalExpense !== undefined && Number(settlementData.totalExpense) > 0
    ...
    : items.reduce((acc, cur) => acc + (Number(cur.amount) || 0), 0);

  const activeCount = items.filter((i) => i.amount > 0).length;
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-[11.5px]">
              {items.map((item) => {
                ...
                return (
                  <tr
                    key={item.code}
                    className={`hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors h-8 ${
                      !isZero ? "bg-white dark:bg-slate-900 font-medium" : "text-slate-400 dark:text-slate-500"
                    }`}
                  >
                    ...
                    <td className={`py-1 px-2.5 text-right font-mono font-black ${
                      !isZero ? "text-emerald-700 dark:text-emerald-300" : "text-slate-400 dark:text-slate-600"
                    }`}>
